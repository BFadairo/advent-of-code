...
public class SonarSweep {
    ...
    public static int Sweep(String filePath) {
        List<Integer> sweeps = readFile(filePath);
        return countPositiveChanges(sweeps);
    }
     ...
     * O(N) Runtime
     * @param sweeps - A List of Sweeps
     * @return - Amount of times next value was greater than previous
     */
    private static int countPositiveChanges(List<Integer> sweeps) {
        int count = 0;

        for (int i = 1; i < sweeps.size(); i++) {
            if (sweeps.get(i) > sweeps.get(i - 1)) count++;
        }
